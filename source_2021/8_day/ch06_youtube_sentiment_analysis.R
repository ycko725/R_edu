...
# 쿠팡 1조원 적자에도 멈출 수 없는 이유(히스토그램) > 
ggplot(cmt_pos_1_cnt) +
...
# 절대로 전세 살지 마라 1부 | 부동산읽어주는남자(히스토그램) > 
ggplot(cmt_pos_2_cnt) +
  geom_histogram(aes(Freq), bins = 100, fill = "#161240", color = "white") +    
  scale_x_continuous(limits = c(0, 200)) +
  scale_y_continuous(limits = c(0, 150)) +
  labs(title = "절대로 전세 살지 마라 1부 | 부동산읽어주는남자") + 
  theme(axis.ticks = element_blank(), 
        axis.title = element_blank(), 
        panel.background = element_blank())
...
ggplot(cmt_pos_3_cnt) +
  ...
  theme(axis.ticks = element_blank(), 
        axis.title = element_blank(), 
        panel.background = element_blank()) 
# 부자는 알지만 가난한 사람은 모르는 것 (존리)(상자 그림)
ggplot(cmt_pos_3_cnt) +
  geom_boxplot(aes(y = Freq)) +
  scale_y_continuous(limits = c(0, 650)) + 
  labs(title = NULL) +
